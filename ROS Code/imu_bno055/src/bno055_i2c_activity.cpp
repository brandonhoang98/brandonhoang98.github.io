...
bool BNO055I2CActivity::spinOnce() {
    ...
    //("Printing: " << msg_data.orientation.y );
    
    //orientation values vary from -1.0 to 1.0 with 0 being the neutral level position
    if (msg_data.orientation.y > 0.2) {
        softPwmWrite(4, 15);
        softPwmWrite(5, 14); 
        ROS_INFO("Right roll detected");
        }
      
    else if (msg_data.orientation.y  < -0.2) {        
        softPwmWrite(4, 14);
        softPwmWrite(5, 15); 
        ROS_INFO("Left roll detected");
        }
