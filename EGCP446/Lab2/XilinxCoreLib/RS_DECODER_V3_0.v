...
//------------------------------------------------------------------------------
// $Id: rs_decoder_unobf_v3_0.v,v 1.4 2002/03/29 16:01:02 janeh Exp $
//

`timescale 1ns/10ps
module RS_DECODER_V3_0(DATA_IN,SYNC,CLK,CE,ERASE,RESET,SR,ERR_CNT,ERR_FOUND,FAIL,BLK_STRT,BLK_END,ERASE_CNT,READY,DATA_OUT,DATA_DEL
);parameter C_CLKS_PER_SYM=1,C_FAMILY_INT=0,C_GEN_START=0,C_H=1,C_HAS_CE=0,C_HAS_DATA_DEL=0,C_HAS_ERASE=0,C_HAS_SR=0,C_HUSET="MOD",C_K=188,C_MEMSTYLE=2
,C_N=204,C_OPTIMISATION=0,C_POLYNOMIAL=0,C_SPEC=0,C_SYMBOL_WIDTH=8,C_SYNC_MODE=0,C_USEHUSET=1,C_USERPM=1;
...
begin for(i=C_N-C_K;i>0;i=i-1)erase_locator[i]=-1;erase_locator[0]=0;end endtask task reset_proc3;integer i;begin for(i=C_N-1;i>=0;i=i-1)begin errors[i
]=-1;symbol_corrected[i]=-1;corrected_message[i]=-1;end for(i=IOLOIL0I0lIL0I0lO11IOL1110;i>=0;i=i-1)begin lambda_var[i]=-1;b_var[i]=-1;temp_poly[i]=-1;error_locator
[i]=-1;lambda_diff_var[i]=-1;end for(i=II0I1OL0IOll111L0llLI10IL1;i>=0;i=i-1)begin omega_var[i]=-1;error_evaluator[i]=-1;end for(i=IIIIl00OII0lOOL0010L1OOLOI-1;i>=0;i=i-1)syndromes[i]=-
1;sync_dd<=0;deg_lambda=0;deg_omega=0;reciprocal=-1;lambda_value=-1;lambda_diff_value=-1;omega_value=-1;substitute=-1;num_errors_var=0;num_erasures=0;
discrep=-1;temp=-1;temp2=-1;temp3=-1;temp4=-1;temp7=-1;temp8=-1;delta=-1;gamma=-1;l=0;deg_cnt=0;err_cnt_out<=0;err_cnt_sig<=0;if(C_HAS_ERASE!=0)begin
 erase_cnt_out<=0;erase_cnt_sig<=0;end err_found_out<=0;err_found_sig<=0;fail_out<=0;fail_sig<=0;blk_strt_out<=0;blk_strt_sig<=0;blk_end_out<=0;
blk_end_sig<=0;ready_out<=1;ready_dc=0;index<=-1;input_count<=-1;init_erase_locator;end endtask task reset_proc2;integer i;begin for(i=C_N-1;i>=0;i=i-1
)begin sym_int[i]=0;r_ap[i]=-1;errors[i]=-1;symbol_corrected[i]=-1;corrected_message[i]=-1;end for(i=IOLOIL0I0lIL0I0lO11IOL1110;i>=0;i=i-1)begin lambda_var[i]=-1;
b_var[i]=-1;temp_poly[i]=-1;error_locator[i]=-1;lambda_diff_var[i]=-1;end for(i=II0I1OL0IOll111L0llLI10IL1;i>=0;i=i-1)begin omega_var[i]=-1;error_evaluator[i]=-1;end
 for(i=IIIIl00OII0lOOL0010L1OOLOI-1;i>=0;i=i-1)syndromes[i]=-1;sync_dd<=0;deg_lambda=0;deg_omega=0;reciprocal=-1;lambda_value=-1;lambda_diff_value=-1;omega_value=-1;
substitute=-1;num_errors_var=0;num_erasures=0;discrep=-1;temp=-1;temp2=-1;temp3=-1;temp4=-1;temp7=-1;temp8=-1;delta=-1;gamma=-1;l=0;deg_cnt=0;index_var
=0;data_out_out<=0;if(C_HAS_DATA_DEL!=0)data_del_out<=0;if(C_HAS_ERASE!=0)begin erase_cnt_out<=0;erase_cnt_sig<=0;end err_cnt_out<=0;err_cnt_sig<=0;
err_found_out<=0;err_found_sig<=0;fail_out<=0;fail_sig<=0;blk_strt_out<=0;blk_strt_sig<=0;blk_end_out<=0;blk_end_sig<=0;ready_out<=1;ready_dc=0;index
<=-1;index_out<=0;input_count<=-1;init_erase_locator;for(i=0;i<=IILL10000OllL0OI1lLI0I0IO0+1;i=i+1)begin funnv_shift[i]<=1'b0;funny_shift[i]<=1'b0;end end endtask task
 reset_code_buf;integer i;begin for(i=0;i<=IILL10000OllL0OI1lLI0I0IO0;i=i+1)code_buff_in[i]<=0;end endtask task reset_proc;begin ce_d<=0;erase_d<=0;data_in_d<=0;sync_d<=0
;sync_db<=0;start<=0;flag_funny<=0;flag_funnv<=0;flag_funny_d<=0;flag_funnv_d<=0;end endtask assign#(IIl0II0O0LII0LlLlIl1LO1O0I)ERR_CNT=err_cnt_out;assign#(
IIl0II0O0LII0LlLlIl1LO1O0I)ERR_FOUND=err_found_out;assign#(IIl0II0O0LII0LlLlIl1LO1O0I)FAIL=fail_out;assign#(IIl0II0O0LII0LlLlIl1LO1O0I)BLK_STRT=blk_strt_out;assign#(IIl0II0O0LII0LlLlIl1LO1O0I)BLK_END=
blk_end_out;assign#(IIl0II0O0LII0LlLlIl1LO1O0I)ERASE_CNT=erase_cnt_out;assign#(IIl0II0O0LII0LlLlIl1LO1O0I)READY=ready_out;assign#(IIl0II0O0LII0LlLlIl1LO1O0I)DATA_OUT=data_out_out;assign#(
IIl0II0O0LII0LlLlIl1LO1O0I)DATA_DEL=data_del_out;always@(CLK or posedge RESET or pwr_on_reset)begin if(RESET||pwr_on_reset)begin sync_bit<=0;sym_clk<=0;flag_funny<=0
;flag_funnv<=0;end else if(C_CLKS_PER_SYM==1)begin flag_funny<=0;flag_funnv<=0;end else begin if(CLK)begin sync_bit<=SYNC;if(sync_bit==0&&SYNC==1)begin
 if(sym_clk!=0)begin if(input_count!=-1)begin if(input_count<IILL10000OllL0OI1lLI0I0IO0-1)$display(
...
// synthesis attribute GENERATOR_DEFAULT of RS_DECODER_V3_0 is "generatecore com.xilinx.ip.rs_decoder_v3_0.rs_decoder_v3_0" 
// box_type "black_box"  
// synthesis attribute box_type of RS_DECODER_V3_0 is "black_box" 
 endmodule
